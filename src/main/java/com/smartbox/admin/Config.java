    ...
    public void reload() {
        ...
        if (config != null) {
            try (final FileInputStream in = new FileInputStream(config)) {
                ...
                logger.warn("1: Can't read the config.properties file. Please check the file.", e);
            }
        }
        ...
        if (properties.isEmpty()) {
            try (final InputStream stream = getClass().getClassLoader().getResourceAsStream("config.properties")) {
                ...
                logger.warn("2: Can't read the config.properties file. Please check the file.", e);
            }
        }
        ...
        if (properties.isEmpty()) {
            try (final InputStream stream = getClass().getResourceAsStream("/config.properties")) {
                ...
                logger.warn("3: Can't read the config.properties file. Please check the file.", e);
            }
        }

        if (!properties.isEmpty()) {
            logger.warn("Reading from config.properties stream is successfully.");
        }
    }
